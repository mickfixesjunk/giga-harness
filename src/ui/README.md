...
| [`api/`](./api/) | — | All REST handlers + their DTOs, split into `mod`/`read`/`mutate`/`dto` (see below). The router references handlers as `api::<fn>`; `api/mod.rs` re-exports them. |
| [`ws.rs`](./ws.rs) | 367 | WebSocket live-tail endpoint: snapshot-then-append wire protocol, per-channel tailer lifecycle. |
| [`channel.rs`](./channel.rs) | ~10 | Thin re-export of the `===`-frame parser — `pub use crate::foundation::frame::{parse_posts as parse, Post}`. |
...
`fn pid_file_path() -> Result<PathBuf>` (mod.rs:57) returns `crate::coordination::cursor::giga_home().join("ui.pid")`, erroring out if neither `$HOME` nor `%USERPROFILE%` is set.
...
**Purpose.** Defines the full route table mapping URLs to handlers in `api/`/`ws.rs`, the async `serve` entrypoint with graceful shutdown, and the two embedded static assets.
...
**Split (Phase 12).** What used to be a single `api.rs` is now a folder:
- [`api/mod.rs`](./api/mod.rs) — re-export façade. The router in `server.rs`
  references handlers as `api::<fn>`, so `mod.rs` `pub use`s every handler (and
  every DTO) verbatim from the submodules; the historical `crate::ui::api::<Type>`
  paths keep resolving.
- [`api/read.rs`](./api/read.rs) — the GET handlers (`list_swarms`, `get_swarm`,
  `get_channel_tail`, `get_swarm_timeline`, `list_processes`, `get_agent_log`) +
  their read DTOs (`SwarmSummary`, `SwarmDetail`, `AgentDto`, `ChannelDto`,
  `TailQuery`, `Timeline`, `TimelinePost`, `ChannelTail`, `LogQuery`,
  `LogSnapshot`, `AgentProcessStatus`).
- [`api/mutate.rs`](./api/mutate.rs) — the POST handlers (`post_to_channel`,
  `set_swarm_archived`, `validate_swarm`, `launch_swarm`, `kill_swarm`,
  `add_agent`, `add_channel`, `run_upgrade`) + their bodies, plus the `run_giga`
  shell-out shim.
- [`api/dto.rs`](./api/dto.rs) — the shared error/exec primitives (`ExecResult`,
  `PostError`) and the `not_found` / `internal` helpers.
...
- `pub async fn post_to_channel(Path((name, file)), Json(PostBody))` — pre-checks the channel is declared, then appends via `crate::coordination::post::run`. Maps `"is not a participant"` / `"WAITING ON target"` errors to **400**, everything else to **500**.
...
**Helpers.** `summarize_swarm`, `detail_from`, `agent_dto`, `agent_has_tmux_window`, `channel_dto`, `last_activity`, `newest_md_mtime`, `inbox_dir_for` (in `read.rs`), `not_found`, `internal` (in `dto.rs`), and the shell-out shim in `mutate.rs`:

```rust
fn run_giga(argv: &[&str]) -> Result<ExecResult, std::io::Error>
```

It resolves the binary via `crate::foundation::self_invoke::giga_binary()` (which falls back to bare `"giga"`) and runs it with **captured** stdout/stderr (never inherited), so the child sees the same binary/behavior as the running server.
...
- `fn resolve_channel_path(swarm, file) -> Result<PathBuf, String>` — registry + config lookup, `"channel not in swarm config"` guard, inbox-side selection (mirrors `api/read.rs`).
...
### `channel.rs` — channel-post parser (re-export)

**Purpose.** Converts an append-only `===`-delimited channel `.md` log into
`Vec<Post>` for the REST endpoints and the WebSocket tailer/snapshot. As of the
foundation refactor this is no longer a private copy of the parser — the grammar
now lives in [`crate::foundation::frame`](../foundation/frame.rs), the **one**
parser shared across `post`/`watch`/`sweep`/`stale_wait`/`ui`. The whole module
is:
...
So `channel::Post` is `foundation::frame::Post { sender, subject, timestamp_iso,
body }` and `channel::parse(content)` is `foundation::frame::parse_posts` —
**oldest-first** frame extraction.

**Gotchas.** The header-detection / em-dash UTF-8-boundary safety, the `[<…`
placeholder guard, and the 20-byte timestamp-tail rule all live in
`foundation::frame` now (with its regression tests). The duplication that this
file used to carry against `watch::is_header_line` is gone — both sides import the
single grammar, so there is no longer a contract to keep mirrored by hand. The
parser still re-runs on every poll tick and REST request (no caching).
...
- `pub fn is_alive(path: &Path) -> bool` — reads + parses the PID and returns `process_alive`; missing/malformed/dead ⇒ `false`. Called from `scaffold::launch`.
...
**Control flow.** `mod.rs::run` calls `acquire` first, so a live prior server makes the call bail before binding. The `Guard` is held for the server lifetime; normal exit / Ctrl-C unwinds and Drop unlinks the file. Separately, `scaffold::launch` calls `crate::ui::pid::is_alive(home.join("ui.pid"))` so `giga launch --ui` skips scheduling a `giga ui` pane when a server is already running, and picks back up after a crash (stale PID).
...
**Inside the folder.** `mod.rs::run` → `pid::acquire` → Tokio runtime → `server::serve` → `build_router().with_state(AppState::new())`. Every read handler in `api/read.rs` is independent and stateless: `registry::load()` → `Config::load(entry.config)` → serialize, using `channel::parse` for post extraction and `process::snapshot()` for liveness. The single shared object is `AppState.tailers`: `ws::ensure_tailer` lazily creates one `broadcast` channel + `run_tailer` polling task per `(swarm, file)`; all WS clients of that channel share it.

**Across folders.**
- [`../dispatch.rs`](../dispatch.rs) dispatches `Command::Ui { bind, port } => ui::run(ui::Args { bind, port })` — the sole entry into this module (`main.rs` is the parse-and-`run` shim).
- [`../registry.rs`](../registry.rs) — `registry::load()` / `Entry { name, config, code_roots, archived }` / `set_archived` back the swarm list and archive toggle (`~/.giga/swarms.toml`).
- [`../config/`](../config/) — `Config::load`, plus fields `agents`, `channels`, `paths.{wsl_inbox, windows_inbox}`, `project.{description, runtime, launch_model}`, `this_host`, `hosts`, and `Config::agent_runtime(a)`.
- [`../coordination/post.rs`](../coordination/post.rs) — `crate::coordination::post::run` is the canonical channel-append invoked by `post_to_channel`; its participant / WAITING-ON validation errors surface as 400s.
- [`../coordination/cursor.rs`](../coordination/cursor.rs) — `cursor::giga_home()` (a thin re-export of `foundation::dirs::giga_home`) resolves `~/.giga` for the PID path here and the `is_alive` check in `scaffold::launch`.
- [`../foundation/frame.rs`](../foundation/frame.rs) — the `===`-frame parser `channel.rs` re-exports as `parse`/`Post`; the single grammar shared with `coordination`.
- [`../foundation/self_invoke.rs`](../foundation/self_invoke.rs) — `giga_binary()` resolves the binary `run_giga` shells out to.
- [`../scaffold/launch.rs`](../scaffold/launch.rs) — calls `crate::ui::pid::is_alive` to decide whether `giga launch --ui` schedules a `giga ui` daemon pane (the cross-command integration point).
- [`../coordination/watch/`](../coordination/watch/) — `process.rs` discovers the `giga watch --as <slug>` processes that the watcher spawns.
- **Subprocesses** — `api/mutate.rs` / `process.rs` shell out to the running `giga` binary (`validate`/`launch`/`kill`/`add-agent`/`add-channel`/`upgrade` via `run_giga` → `foundation::self_invoke::giga_binary`), plus `tmux` (`kill-session`, `capture-pane`, `list-sessions`/`list-windows`) and `ps`.
...
- [`../dispatch.rs`](../dispatch.rs) — CLI dispatch (`Command::Ui`); [`../cli.rs`](../cli.rs) — the `Ui` clap variant.
- [`../registry.rs`](../registry.rs), [`../config/`](../config/), [`../coordination/post.rs`](../coordination/post.rs), [`../scaffold/launch.rs`](../scaffold/launch.rs), [`../coordination/watch/`](../coordination/watch/), [`../coordination/cursor.rs`](../coordination/cursor.rs), [`../foundation/frame.rs`](../foundation/frame.rs) — collaborating modules.
